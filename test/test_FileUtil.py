...
from helpers.constants import DataPath, OrthosPath, PathDef, XMLPath
...
class TestFileUtil(unittest.TestCase):
    def setUp(self):
        ...
        self.bac_file = str(PathDef(dir=os.sep.join([os.getcwd(), "test", "test_files", ""]), filename="bac_test.fas"))

    def tearDown(self):
        if os.path.exists(str(self.bac_file)):
            shutil.rmtree(str(OrthosPath("")))
            shutil.rmtree(str(XMLPath("")))
            shutil.rmtree(str(DataPath("")))
            os.remove(str(self.bac_file))
    ...
    def test_merging_multiple_fastas(self):
        FileUtil.merge_domain_fastas(self.bac_file, self.accession_dict)
        self.assertEqual(os.path.getsize(self.bac_file), os.path.getsize(self.test_path + "sample.fas"))
