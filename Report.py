...
def generateReport(name, quer, models, dom):
    ...
    ret.write(
        'BLASTs are performed using expected value (E-value) thresholds based on the kingdom (Bacteria, Archaea, and Eukaryota).\n' +
    ...
    ret.write(
        'Organism                       Kingdom     Accession No.(GI)      No. on list/length of list     No. on list/length of list         ')

    ret.write('Seq Definition\n')
    # print info
    keys = list(info.keys())
    keys.sort()
    length = SeqUtil.find_longest_key(info) + 6
    for i in keys:
        # print info[i]
        # writing organism
        ret.write(i)
