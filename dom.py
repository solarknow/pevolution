    ...
    for mod, params in original_models.items():
        ...
        if not os.path.exists(str(MLPath(f"{prefix}_phyml_boot_trees.txt"))):
            ...
            if params[0] == "0" and params[1] == "0":
                ...
                run_phyml(MLPath(prefix), mod.split("+")[0], AlignsPath(f"{dom}-{out}.best.dnd"), v=params[1])
            elif params[1] == "0":
                print("Adding gamma shape")
                run_phyml(MLPath(prefix), mod.split("+")[0], AlignsPath(f"{dom}-{out}.best.dnd"), a=params[0])
            else:
                print("Adding both proportion of invariable sites and gamma shape")
                run_phyml(
                    MLPath(prefix), mod.split("+")[0], AlignsPath(f"{dom}-{out}.best.dnd"), a=params[0], v=params[1]
                )
    print("Done Pamling")
print("Getting ready for building a tree")
if not os.path.exists(str(BayesPath(f"{dom}-{out}-bayes.nxs"))):
    SeqUtil.bayesfile(AlignsPath(f"{dom}-{out}.best.nex"), original_models, BayesPath(f"{dom}-{out}-bayes.nxs"))
